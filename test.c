...
static bool testCase(const char* const string, const int* errorCode, const char* const rightString, const size_t number)
{
    char* finalString = (char*)calloc(strlen(string), sizeof(char));
    finalString = stringProcessing(string, finalString, errorCode);
    if (strcmp(finalString, rightString) || *errorCode != OK_CODE)
    {
